...
from src.data.config import MASTER_ADMIN, PHOTO_GALLERY_PATH, PHOTO_EXTENSION
from src.data.enums import CallbackData, BotMessageText, Symbols, CacheKeys
from src.data.processing import process_input
...
async def get_photo(message: types.Message, state: FSMContext):
    ...
    if message.content_type != 'document':
        ...
        async with state.proxy() as data:
            ...
            data['photo'] = PHOTO_GALLERY_PATH + file_name
            # save photo in the gallery
            await message.document.download(data['photo'])
            # delete answer
            await message.delete()
