...
from src.data.config import MASTER_ADMIN
from src.data.enums import CallbackData, BotMessageText, Symbols, CacheKeys
from src.data.processing import transform_name, process_input
from src.data.validation import check_integer
        ...
        async with state.proxy() as data:
            ...
            if data['section'] != CallbackData.photo.value:
                ...
                if message.content_type == 'photo':
                    ...
                    await message.document.download(data['chosen_doctor'][CallbackData.photo.value])
                    # log photo update
                    logger.info(
            ...
            if len(data['specialities']) == len(data['specialities_pool']):
                ...
                if data['user_uid'] == MASTER_ADMIN or data['user_uid'] in admins:
                    ...
                    for speciality in data['specialities']:
                        ...
                        doctors = await query.get_doctors_by_speciality(id=speciality_id)
                        # check if there are any doctors left
                        if len(doctors) == 0:
                            # delete speciality if there is no doctors left
                            await query.delete_speciality(speciality_id=speciality_id)
