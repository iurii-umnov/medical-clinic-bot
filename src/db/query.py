...
async def get_or_create_profile(tg_uid: int, username: str, full_name: str, phone: str) -> UserProfile:
    async with get_async_session() as session:
        query = sa.select(UserProfile).filter(
            UserProfile.tg_uid == tg_uid,
...
async def create_appointment(tg_uid: int, username: str, full_name: str, phone: str,
                             consultation_type: str, communication_type: str,
                             user_request: str, doctor_id: int, preferable_dt: str) -> Appointment:
    async with get_async_session() as session:
...
async def create_callback(tg_uid: int, username: str, full_name: str, phone: str) -> CallBack:
    async with get_async_session() as session:
        profile = await get_or_create_profile(
            tg_uid=tg_uid,
            username=username,
...
async def create_feedback(tg_uid: int, username: str, message: str) -> Feedback:
    async with get_async_session() as session:
        query = sa.select(UserProfile) \
            .filter(UserProfile.tg_uid == tg_uid,
                    UserProfile.username == username) \
...
async def calculate_statistic(table: Base, start_date: Union[str, datetime],
                              end_date: Union[str, datetime], consultation_type: str = None):
    async with get_async_session() as session:
